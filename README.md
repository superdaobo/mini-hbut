...
- `main`：新主线（当前持续开发与发布分支）。
- `old`：旧主线归档分支（由发布脚本在推送前自动覆盖更新）。
...
python release.py --no-archive-old  # 不归档 old（默认会归档）
```
...
> 当前推荐策略：**保留教务/业务命令在现有后端（Rust/服务端）实现，设备能力走 Capacitor 原生插件**，采用渐进迁移而不是一次性重写。  
> 原因：业务链路（CAS 登录、教务接口解析、会话管理）复杂且稳定优先；移动端通知/分享/权限/后台能力更适合用 Capacitor 原生层补强。

### 1) 架构分层（必须遵守）

- **业务层**：成绩、课表、考试、电费、图书等业务命令，优先保留现有后端实现，不在页面层重复造轮子。
- **设备能力层**：通知、外链、文件分享、息屏保活、权限等，统一走 `src/platform` 桥接层。
- **页面层**：仅调用 `platformBridge` 或统一 `utils`，禁止直接依赖 `@tauri-apps/*` 或 `@capacitor/*`。

### 2) 能力新增规范

新增跨端能力必须按顺序改造：

1. 在 `src/platform/types.ts` 增加接口定义。
2. 在 `src/platform/adapters/web.ts`、`src/platform/adapters/tauri.ts`、`src/platform/adapters/capacitor.ts` 分别实现。
3. 在 `src/platform/index.ts` 透出统一入口。
4. 页面/工具层仅替换为桥接调用，不做平台分叉硬编码。

### 3) 运行时与兼容策略

- 使用 `src/platform/runtime.ts` 识别 `web/tauri/capacitor`，禁止在业务组件重复判断容器类型。
- Tauri 场景可保留 Rust 命令兜底（如 ACL 受限时的通知权限命令），但必须封装在桥接或模块内。
- 移动端优先保证可用性：远程配置失效时需有本地兜底配置。

### 4) 代码质量与提交要求

- 关键流程写中文注释（尤其平台差异、降级策略、失败兜底）。
- 禁止把临时调试文件、导出文件、抓包文件提交到主分支。
- 每次迁移提交必须附带构建验证结果：
  - `npm run build`
  - `npm run cap:sync`
  - `npm run tauri build -- --no-bundle`（或对应平台最小构建）

### 5) 迁移优先级建议

- **优先迁移**：通知、权限、分享、外链、文件系统、后台保活（移动端体验相关）。
- **暂缓重写**：教务抓取、登录会话、复杂解析逻辑（先保持稳定，后续按模块拆分服务化）。
- **长期目标**：业务能力逐步服务化（HTTP API），壳层只负责设备能力与 UI。
...
1. 桌面端继续使用 `Tauri`（保持现有发布链路）。  
2. 移动端新增 `Capacitor` 外壳（iOS/Android 深度能力逐步迁移）。  
3. 前端业务层通过 `src/platform` 统一桥接，减少平台耦合。
...
- `ios/`：iOS 原生工程
- `src/platform/`：平台桥接层（tauri/capacitor/web）

### 当前状态

- ✅ 已完成 Web 构建（`npm run build`）
- ✅ 已完成 Capacitor 同步（`npx cap sync`）
- ✅ 已生成 Android/iOS 原生工程骨架
- ⚠️ iOS 原生编译需在 macOS + Xcode 环境执行
