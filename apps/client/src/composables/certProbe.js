 ...
 * （设备时间不准也会导致过期误报）。
 *
 * @param {Array<{domain: string, status: string}> | null | undefined} results
 * @returns {string[]} 提示文案列表
 */
export function certIssueMessagesFromResults(results) {
  if (!Array.isArray(results)) return []
  return results
    .filter((item) => item && item.status === 'cert-error')
 ...
 */
export function ensureCertProbe() {
  if (probePromise) return probePromise
  probePromise = invokeNative('probe_school_cert_status')
    .then((results) => {
...
/**
 * 「我的」页面横幅状态入口。
 * @returns {{ certIssues: import('vue').Ref<string[]>, ensureCertProbe: () => Promise<unknown> }}
 */
