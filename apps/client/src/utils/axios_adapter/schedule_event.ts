...
import { emitScheduleEventChanged, type ScheduleEventChangeReason } from '../schedule_event_signal';
 ...
 */
const fireScheduleEventReconcile = (
  data: JsonObject,
  ...
  emitScheduleEventChanged(sid, reason);
};
 ...
 * 抽取前 post.ts 已 988 行）。各端点分支的写法与既有 custom course 分支保持一致：
...
export const handleScheduleEventPost = async (
  ...
  if (url.includes('/v2/schedule/event/list-all')) {
    try {
      if (hasTauri) {
        const payload = await invoke('list_schedule_events_all', {
          studentId: data?.student_id || data?.studentId || ''
        });
        return mockResponse(payload);
      }
      const res = await bridgePost('/schedule/event/list-all', data || {});
      if (res?.success && res?.data) {
        return mockResponse({ success: true, ...res.data });
      }
      return mockResponse({ success: false, error: errorMessage(res.error) || '获取日程失败' });
  ...
  if (url.includes('/v2/schedule/event/replace-all')) {
    try {
      if (hasTauri) {
        const payload = await invoke('replace_schedule_events', { req: data || {} });
        fireScheduleEventReconcile(data, 'cloud-restore');
        return mockResponse(payload);
      }
      const res = await bridgePost('/schedule/event/replace-all', data || {});
      if (res?.success && res?.data) {
        fireScheduleEventReconcile(data, 'cloud-restore');
        return mockResponse({ success: true, ...res.data });
      }
      return mockResponse({ success: false, error: errorMessage(res.error) || '恢复日程失败' });
    } catch (err) {
      return mockResponse({ success: false, error: errorMessage(err) });
    }
  }
