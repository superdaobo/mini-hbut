...
impl ElectricityModule {
    ...
    pub async fn query_location(
        ...
        let token = self.auth_token.as_ref().ok_or("未设置电费查询授权 Token")?;
    ...
    pub async fn query_account(
        &self,
        payload: &Value,
    ) -> Result<ElectricityBalance, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let template_list = result_data.get("templateList").and_then(|v| v.as_array());
    ...
    pub async fn get_root_areas(
        &self,
    ) -> Result<Vec<LocationItem>, Box<dyn std::error::Error + Send + Sync>> {
