...
impl TrainingPlanModule {
    ...
    pub async fn fetch_options(
        &self,
    ) -> Result<TrainingPlanOptions, Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!(
            "[调试] Training plan page status: {}, 地址: {}",
            status, final_url
        );
    ...
    pub async fn fetch_jys(
        &self,
        yxid: &str,
    ) -> Result<Vec<SelectOption>, Box<dyn std::error::Error + Send + Sync>> {
        if yxid.is_empty() {
            return Ok(vec![]);
        }
    ...
    pub async fn fetch_courses(
        ...
        let response = self
            ...
            .header(
                "Referer",
                format!("{}/admin/xsd/studentpyfa", JWXT_BASE_URL),
            )
            .send()
            .await?;
        ...
        let final_url = response.url().to_string();
        println!(
            "[调试] Training plan courses status: {}, 地址: {}",
            status, final_url
        );
    ...
    fn parse_options(select_html: &str) -> Vec<SelectOption> {
        ...
        if let Some(re) = re {
            for cap in re.captures_iter(select_html) {
                ...
                let clean_label = decode_html_entities(&clean_label)
                    .to_string()
                    .trim()
                    .to_string();
                ...
                if clean_value.is_empty()
                    && (clean_label == "请选择" || clean_label == "全部" || clean_label.is_empty())
                {
                    continue;
                }
