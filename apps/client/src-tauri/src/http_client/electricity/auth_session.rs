...
//!
//! 负责：
//! - SSO 跳板（融合门户 -> 一码通）换取 tid/ticket，再交换电费 token
//! - refreshToken 刷新与本地会话缓存（含风控冷却保护）
//! - 一码通 token 独立获取与浏览器 tid 签发
//! - code.hbut.edu.cn 专项重登流程

use crate::http_client::{HbutClient, AUTH_BASE_URL};
...
impl HbutClient {
    pub(super) fn get_cached_electricity_token_fast(&self) -> Option<String> {
        let token = self
            .electricity_token
            .clone()
        ...
        if let Some(exp) = self.electricity_token_expires_at.clone() {
            if exp > now + ChronoDuration::seconds(20) {
                return Some(token);
            }
            return None;
        }
    ...
    pub(crate) async fn get_electricity_token(
        &mut self,
        ...
        match self.client.get(sso_url).send().await {
            Ok(resp) => {
                ...
                if let Some(token_header) = resp.headers().get("Authorization") {
                    ...
                    println!(
                        "[调试] 直连 SSO 获取令牌头: {}...",
                        &auth_token.chars().take(30).collect::<String>()
                    );
                }
                ...
                if tid.is_empty() && ticket.is_empty() && auth_token.is_empty() {
                    ...
                    if ticket.is_empty() {
                        if let Some(caps) =
                            regex::Regex::new(r#"ticket=([^&"']+)"#)?.captures(&html)
                        {
                            ticket = caps
                                .get(1)
                                .map(|m| m.as_str().to_string())
                ...
                while redirect_count < MAX_重定向S {
                        ...
                        .await
                    {
                        ...
                        Err(err) => {
                            ...
                            if current_url.contains("authserver/login")
                                || current_url.contains("auth.hbut.edu.cn")
                            {
                                needs_login = true;
                                break;
                            }
                    ...
                    if status.is_redirection() {
                        if let Some(location) = response.headers().get("Location") {
                            ...
                            if ticket.is_empty() {
                                if let Some(caps) =
                                    regex::Regex::new(r"ticket=([^&]+)")?.captures(location_str)
                                {
                                    ticket = caps
                                        .get(1)
                                        .map(|m| m.as_str().to_string())
                        ...
                        if tid.is_empty() {
                            if let Some(caps) =
                                regex::Regex::new(r#"tid=([^&"']+)"#)?.captures(&html)
                            {
                                tid = caps
                                    .get(1)
                                    .map(|m| m.as_str().to_string())
                        ...
                        if auth_token.is_empty() {
                            if let Some(caps) =
                                regex::Regex::new(r"(C2CDB[0-9A-F]{10,}(?:\.[0-9A-Za-z]+){2,})")?
                                    .captures(&html)
                            {
                                auth_token = caps
                                    .get(1)
        ...
        if auth_token.is_empty() && (!tid.is_empty() || !ticket.is_empty()) {
            ...
            let url_get2 = format!(
                "https://code.hbut.edu.cn/server/auth/getToken?tid={}&orgId=2",
                tid
            );
            ...
            if !tid.is_empty() {
                ...
                token_urls.push((
                    url_post.clone(),
                    Some(serde_json::json!({"tid": tid, "orgId": 2})),
                ));
            }
            if !ticket.is_empty() {
                token_urls.push((
                    format!(
                        "https://code.hbut.edu.cn/server/auth/getToken?ticket={}",
                        ticket
                    ),
                    None,
                ));
                token_urls.push((
                    format!(
                        "https://code.hbut.edu.cn/server/auth/getToken?ticket={}&org=2",
                        ticket
                    ),
                    None,
                ));
                token_urls.push((
                    format!(
                        "https://code.hbut.edu.cn/server/auth/getToken?ticket={}&orgId=2",
                        ticket
                    ),
                    None,
                ));
                token_urls.push((
                    url_post.clone(),
                    Some(serde_json::json!({"ticket": ticket, "orgId": 2})),
                ));
            }
                            ...
                            .unwrap_or(false)
                        {
                            if let Some(result_data) = json.get("resultData") {
                                auth_token = result_data
                                    ...
                                    .map(|s| s.to_string())
                                    .unwrap_or_default();
                                refresh_token = result_data
                                    .get("refreshToken")
                                    .and_then(|v| v.as_str())
                                    .map(|s| s.to_string());
                                expires_in = result_data
        ...
        if auth_token.is_empty() {
            if let Some(caps) =
                regex::Regex::new(r"(C2CDB[0-9A-F]{10,}(?:\.[0-9A-Za-z]+){2,})")?.captures(&cookies)
            {
                auth_token = caps
                    .get(1)
                    .map(|m| m.as_str().to_string())
        ...
        if auth_token.is_empty() {
            ...
            return Err(format!(
                "无法获取电费 Authorization Token (tid={}, ticket={})",
                tid, ticket
            )
            .into());
        }
    ...
    pub(super) async fn refresh_electricity_token(
        &mut self,
    ) -> Result<ElectricityTokenBundle, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for body in payloads {
                ...
                .unwrap_or(false)
            {
                ...
                let refresh = result
                    ...
                    .map(|s| s.to_string())
                    .or_else(|| {
                        self.electricity_refresh_token
                            .clone()
    ...
    /// 确保电费 token 可用（必要时刷新）
    pub async fn ensure_electricity_token(
        &mut self,
    ...
    /// 获取一码通 token（独立流程）
    pub async fn get_one_code_token(
        &mut self,
        ...
        if !tid.is_empty() {
            token_urls.push((
                format!("https://code.hbut.edu.cn/server/auth/getToken?tid={}", tid),
                None,
            ));
            token_urls.push((
                format!(
        ...
        for (url, body) in token_urls {
            ...
            if success {
                ...
                let access = result_data
                    ...
                    .unwrap_or_default();
                if !access.is_empty() {
                    let refresh = result_data
                        .get("refreshToken")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                    let expires_in = result_data
                        .get("accessTokenExpire")
                        .and_then(|v| v.as_i64());
    ...
    /// 从一码通返回体里提取 token 字段（供交易/校园码子模块复用）
    pub(super) fn extract_one_code_token(one_code: &serde_json::Value) -> Option<String> {
    ...
    async fn relogin_for_code_service(
        ...
        println!(
            "[调试] relogin_for_code_service 完成。Cookies: {}",
            self.get_cookies()
        );
        Ok(true)
    }
...
mod auth_session_tests {
    ...
    #[test]
    fn extract_access_token_from_result_data() {
        let one_code = serde_json::json!({
            "success": true,
            "resultData": { "accessToken": "C2CDB-TEST-TOKEN" }
        });
        assert_eq!(
            HbutClient::extract_one_code_token(&one_code),
            Some("C2CDB-TEST-TOKEN".to_string())
        );
    }

    #[test]
    fn extract_token_from_result_data() {
        let one_code = serde_json::json!({
            "success": true,
            "resultData": { "token": "fallback-token" }
        });
        assert_eq!(
            HbutClient::extract_one_code_token(&one_code),
    ...
    #[test]
    fn extract_access_token_from_root() {
        let one_code = serde_json::json!({
            "success": true,
    ...
    #[test]
    fn extract_none_when_token_missing() {
        let one_code = serde_json::json!({ "success": false, "message": "err" });
        assert_eq!(HbutClient::extract_one_code_token(&one_code), None);
    }

    #[test]
    fn cached_token_none_when_unset() {
        let client = HbutClient::new();
        assert_eq!(client.get_cached_electricity_token_fast(), None);
    }

    #[test]
    fn cached_token_returns_within_expiry() {
        let mut client = HbutClient::new();
        client.electricity_token = Some("tok-1".to_string());
        client.electricity_token_expires_at = Some(Utc::now() + ChronoDuration::seconds(120));
        assert_eq!(
            client.get_cached_electricity_token_fast().as_deref(),
            Some("tok-1")
        );
    }

    #[test]
    fn cached_token_expired_returns_none() {
        let mut client = HbutClient::new();
        client.electricity_token = Some("tok-2".to_string());
        client.electricity_token_expires_at = Some(Utc::now() - ChronoDuration::seconds(30));
        assert_eq!(client.get_cached_electricity_token_fast(), None);
    }

    #[test]
    fn cached_token_falls_back_to_instant_within_ttl() {
        let mut client = HbutClient::new();
        client.electricity_token = Some("tok-3".to_string());
        client.electricity_token_at = Some(std::time::Instant::now());
        assert_eq!(
            client.get_cached_electricity_token_fast().as_deref(),
            Some("tok-3")
        );
