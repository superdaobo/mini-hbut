...
describe('campus map contract', () => {
  ...
  it('wires campus map direction proxy through vite and tauri bridge', () => {
    ...
    // 原生 Tauri 必须 resolve 到 loopback，不能只靠相对路径
    expect(walking).toContain('resolveCampusMapDirectionUrl')
    expect(walking).toContain('127.0.0.1:4399')
    expect(walking).toContain('isTauriRuntime')
  })
  ...
  it('uses MarkerStyle for building pins so Tencent MultiMarker is visible', () => {
    ...
    const composable = read('src/features/campus-map/composables/useCampusMap.ts')
    expect(markers).toContain('buildCampusBuildingMarkerStyles')
    expect(markers).toContain('MarkerStyle')
    expect(markers).toContain('plainMarkerStyle')
    ...
    expect(controller).toContain("styleId: 'route'")
    expect(composable).toContain('maximumAge: 0')
    expect(composable).toContain('await refreshLocation()')
  })
})
