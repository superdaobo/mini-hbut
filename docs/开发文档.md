...
本文档用于说明 tauri-app 客户端（`apps/client/`）的本地开发、调试、构建与常见问题排查流程。

> 客户端（Vue 前端 + Tauri Rust 后端 + Capacitor 原生工程）已整体迁移至 `apps/client/`。
> 本文档中所有路径均以 `apps/client/` 为根；所有 npm 命令均在 `apps/client/` 目录下执行。
