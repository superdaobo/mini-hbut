...
import { q as detectRuntime, a as isTauriRuntime, _ as __vitePreload, b as isLikelyAndroidUserAgent, $ as isLikelyIOSUserAgent, a0 as isTauriDesktopRuntime } from "./runtime-bridge-BU9jeOXP.js";
  ...
  if (canUseTauriEmbeddedWebview()) {
    try {
      const { getCurrentWindow } = await __vitePreload(async () => {
        const { getCurrentWindow: getCurrentWindow2 } = await import("./runtime-bridge-BU9jeOXP.js").then((n) => n.aa);
...
const forceCloseSchoolWebsiteEmbed = async () => {
  ...
  try {
    const { pushDebugLog } = await __vitePreload(async () => {
      const { pushDebugLog: pushDebugLog2 } = await import("./runtime-bridge-BU9jeOXP.js").then((n) => n.a5);
...
const invokeNative = async (command, args) => {
  const core = await __vitePreload(() => import("./runtime-bridge-BU9jeOXP.js").then((n) => n.a8), true ? __vite__mapDeps([0,1]) : void 0, import.meta.url);
