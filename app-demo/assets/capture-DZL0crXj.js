...
const waitForCaptureReady = async (rootEl) => {
  if (!rootEl) return;
...
const resolveCaptureBackgroundColor = (target, explicitBackgroundColor) => {
  ...
  for (const candidate of candidates) {
    const color2 = readComputedStyle(candidate).backgroundColor;
    if (!isTransparentBackground(color2)) return color2;
  }
...
const captureElementToBlob = async ({
  ...
  try {
    ...
    const resolvedBackgroundColor = resolveCaptureBackgroundColor(target, backgroundColor2);
...
const renderElementToCanvas = async (element, {
  exportWidth,
  exportHeight,
  backgroundColor: backgroundColor2 = DEFAULT_LIGHT_CAPTURE_BACKGROUND,
  scale
} = {}) => {
