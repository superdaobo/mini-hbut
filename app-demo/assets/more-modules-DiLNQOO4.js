import { i as isTestAccountSession, a as isTauriRuntime, b as isLikelyAndroidUserAgent, _ as __vitePreload, t as toNativeFileSrc, g as getNativeAppVersion, p as pushDebugLog, c as isCapacitorRuntime } from "./runtime-bridge-BDjl3Bnt.js";
const IS_APP_STORE_BUILD = String(
  ""
) === "1";
let appStoreBuildOverride = null;
let appStoreSessionOverride = null;
function setAppStoreBuildOverrideForTests(value) {
  appStoreBuildOverride = value;
}
function setAppStoreSessionOverrideForTests(value) {
...
const EXCLUDED_HIDDEN_VIEW_IDS = Object.freeze(
  /* @__PURE__ */ new Set([])
);
...
function isDeepLinkAllowed(viewOrModule, session) {
...
function isRemoteExecutableAllowed() {
  const p = getFeaturePolicy();
  return p.remoteCode || p.remoteModules;
}
function isHotUpdateAllowed() {
...
const fetchJsonViaCapacitor = async (url, timeoutMs = DEFAULT_REMOTE_JSON_TIMEOUT_MS) => {
  const core = await __vitePreload(() => import("./runtime-bridge-BDjl3Bnt.js").then((n) => n.a4), true ? [] : void 0, import.meta.url);
...
const probeOpenUrl = async (url, timeoutMs = FAST_REMOTE_OPEN_PROBE_TIMEOUT_MS) => {
...
const invokeNativeBridge = async (command, args, label = "") => {
  const core = await __vitePreload(() => import("./runtime-bridge-BDjl3Bnt.js").then((n) => n.aa), true ? [] : void 0, import.meta.url);
  try {
...
const deleteModuleState = (moduleId) => {
  const id = safeText(moduleId);
  if (!id) return false;
  const map = readModuleStateMap();
...
const deleteCachedManifestSnapshot = (url) => {
  const cacheKey = buildManifestCacheKey(url);
...
const pickFirstReachableUrl = async (candidates = [], probe = probeOpenUrl) => {
  const urls = toUniqueTextList(candidates);
  ...
  return await new Promise((resolve) => {
    ...
    let pending = urls.length;
    urls.forEach((candidate) => {
      Promise.resolve(probe(candidate, FAST_REMOTE_OPEN_PROBE_TIMEOUT_MS)).then((ok) => {
...
const fetchModuleManifest = async (manifestUrl, options = {}) => {
...
const prepareCapacitorLocalModuleBundle = async ({
  ...
  const { unzipSync } = await __vitePreload(async () => {
    const { unzipSync: unzipSync2 } = await import("./browser-BFKMpcPC.js");
...
const prepareModuleBundle = async ({ channel, moduleInfo, manifest }) => {
  ...
  if (canUseLocalModuleBridgePreview()) {
    try {
      ...
      throw new Error(safeText(error2?.message || error2) || "模块本地准备失败");
    }
  }
  if (isCapacitorRuntime()) {
...
export {
  isLiveLocationAllowed as A,
  app_store_policy as B,
  ...
  canUseLocalModuleBridgePreview as d,
  isLocalModuleBridgePreviewUrl as e,
  isAppStoreBuild as f,
  getUpdateCheckMode as g,
  getFeaturePolicy as h,
  isViewAllowed as i,
  isModuleAllowed as j,
  filterAllowedModules as k,
  NON_OFFICIAL_DISCLAIMER_EN as l,
  isSponsorEntryAllowed as m,
  normalizeModuleHostSessionPayload as n,
  ICP_BEIAN_URL as o,
  SUPPORT_DOCS_URL as p,
  PROJECT_HOME_URL as q,
  resolveModuleHostPreviewSource as r,
  shouldApplyAppStoreRestrictions as s,
  resolveModuleChannel as t,
  fetchModuleCatalog as u,
  getLocalModuleState as v,
  fetchModuleManifest as w,
  deleteModuleState as x,
  deleteCachedManifestSnapshot as y,
  prepareModuleBundle as z
};
