...
//
// 本文件依赖 android.content.Context，仅真机/模拟器编译（#612 集成进 app 工程时随 android/ 源码复制）；
// JVM 单测工程（build.gradle.kts）排除本文件。真实 WorkManager 调度由 #612 实现，
// 骨架阶段 runNow 只返回 synthetic 摘要并更新 native 状态，不执行真实成绩检查。
...
object HbutBackgroundPlugin {
    ...
    fun runNow(context: Context, scope: String?, forceSynthetic: Boolean): String {
        ...
        return try {
            ...
            if (forceSynthetic) {
                ...
                // 骨架阶段无真实业务：返回 synthetic 摘要；#612 接入 WorkManager 真实检查。
                RunSummary.synthetic("Android native 执行成功（骨架 synthetic，真实业务由 #612 接入）")
                    .toJson().toString()
            }
        } catch (e: Exception) {
    ...
    /** disable：关闭调度（骨架阶段仅落盘）；返回状态 JSON。 */
    @JvmStatic
    ...
    /** consumeEvents：消费 native inbox（骨架阶段 native 不产生事件，返回空结果）。 */
    @JvmStatic
    ...
    /** clearContext：按 scope 清理 context/state/events（账号切换/退出）。 */
    @JvmStatic
