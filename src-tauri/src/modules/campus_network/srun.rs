...
pub async fn srun_login(
    ...
    let token = challenge
        ...
        .ok_or_else(|| {
            challenge
                .error_msg
                .unwrap_or_else(|| "未获取 challenge token".to_string())
        })?;
    ...
    let success = portal.access_token.as_ref().is_some_and(|s| !s.is_empty());
    let message = portal.suc_msg.or(portal.error_msg).unwrap_or_else(|| {
        if success {
            "Srun 登录成功".to_string()
        } else {
            "Srun 登录失败".to_string()
